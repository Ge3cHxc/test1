...
# All rights reserved ©AleXa OweNs @AleXa OweNs99X t.me/AleXa OweNs99X
# Get more about devaoloper https://AleXa OweNs.ml
*/
        ...
        if (config.WORKTYPE == 'public') {
            if (config.LANG == 'EN' || config.LANG == 'SI') {
                ...
                if (conn.user.jid === '994775555555@s.whatsapp.net' ) {
                    ...
                    await conn.sendMessage(conn.user.jid, '*This is AleXa OweNs*\n\n_Please do not try plugins here. This is your LOG number._\n_You can try commands to any chat :)_\n\n\n*Thanks for using AleXa OweNs*', MessageType.text);
                    ...
                    if (commits.total === 0) {
                        ...
                        commits['all'].map(
                            (commit) => {
                                degisiklikler += '🔸 [' + commit.date.substring(0, 10) + ']: ' + commit.message + ' <' + "AleXa OweNs" + '>\n';
                            }
                        );
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    if (sendMsg) {
                        ...
                        try {
                            ...
                            if (config.LANG == 'EN' || config.LANG == 'SI') {
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [AleXa OweNs] --*' + 
                                    '\n*AleXa OweNs getting an error!*'+
                                    '\n_This error log may include your number or the number of an opponent. Please be careful with it!_' +
                                    '\n_You can write to our Admins' +
                                    '\n_This message should have gone to your number (saved messages)._\n\n' +
                                    '*Error:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false}
                                );
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '* ERROR ANALYSIS [AleXa OweNs] *' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [AleXa OweNs] --*' + 
                                    '\n*AleXa OweNs getting an error!*'+
                                    '\n_This error log may include your number or the number of an opponent. Please be careful with it!_' +
                                    '\n_You can write to our Telegram group for Admins' +
